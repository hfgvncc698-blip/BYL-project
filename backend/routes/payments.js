...
function requireAdminKey(req, res, next) {
  ...
  if (!expected)
    return res
      .status(500)
      .json({ error: "ADMIN_SEARCH_KEY missing in env" });
...
============================================================ */
async function ensureCustomerIdForUser({ uid, email }) {
  const db = admin.firestore();
  ...
  if (!customerId) {
    const found = await ensureStripe().customers.list({
      email: safeEmail,
      limit: 1,
    });
...
router.post("/admin/set-trial", requireAdminKey, async (req, res) => {
  try {
    ...
    if (!uid) return res.status(400).json({ error: "uid required" });
    if (!Number.isFinite(nbDays) || nbDays < 0)
    ...
    const trialEnd = new Date(now + nbDays * 24 * 60 * 60 * 1000);

    let stripeUpdated = null;
    if (u.stripeSubscriptionId) {
      try {
        const unixTrialEnd = Math.floor(trialEnd.getTime() / 1000);
    ...
    await upsertUserSubscription(String(uid), {
      subscriptionStatus: nbDays > 0 ? "trialing" : (u.subscriptionStatus || "canceled"),
      trialStart,
      trialEnd: nbDays > 0 ? trialEnd : null,
      trialStatus: nbDays > 0 ? "running" : "none",
      hasActiveSubscription: nbDays > 0 ? true : (u.hasActiveSubscription || false),
    });
  ...
  try {
    ...
    if (!chosenPriceId) {
      return res.status(400).json({
        ...
        hint:
          "Vérifie .env : PRO => PRICE_PRO_MONTHLY/PRICE_PRO_YEARLY (ou *_ID) ; " +
...
/* ============================================================
   7) Diagnostics
============================================================ */
router.get("/_diag/stripe-key", (_req, res) => {
  if (!RAW_STRIPE_KEY) return res.status(500).json({ error: "NO_STRIPE_KEY" });
  res.json({
    prefix: RAW_STRIPE_KEY.slice(0, 12),
    last4: RAW_STRIPE_KEY.slice(-4),
