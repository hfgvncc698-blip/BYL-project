...
/* =========================
   ✅ Program name (SAME as ProgramBuilder)
   ========================= */
...
async function resolveCoachNameFromCreatedBy(createdBy) {
  try {
    ...
    if (snap.exists()) {
      const d = snap.data() || {};
      const full = [String(d.prenom || "").trim(), String(d.nom || "").trim()].filter(Boolean).join(" ").trim();
      if (full) return full;
...
const buildInfosFromExercise = (ex) => {
  const values = {
    series: pickFirst(ex, FIELD_MAP.series),
    repetitions: pickFirst(ex, FIELD_MAP.repetitions),
    repos: pickFirst(ex, FIELD_MAP.repos),
    temps: pickFirst(ex, FIELD_MAP.temps),
    charge: pickFirst(ex, FIELD_MAP.charge),
    intensite: pickFirst(ex, FIELD_MAP.intensite),
    watts: pickFirst(ex, FIELD_MAP.watts),
    inclinaison: pickFirst(ex, FIELD_MAP.inclinaison),
    calories: pickFirst(ex, FIELD_MAP.calories),
    tempo: pickFirst(ex, FIELD_MAP.tempo),
    vitesse: pickFirst(ex, FIELD_MAP.vitesse),
    distance: pickFirst(ex, FIELD_MAP.distance),
  };
...
function totalTime(session) {
  ...
  const addEx = (ex) => {
    ...
    const restDefault = toSeconds(pickFirst(ex, FIELD_MAP.repos) ?? 0);
    const series = Number(pickFirst(ex, FIELD_MAP.series) ?? 0) || 1;
    const reps = Number(pickFirst(ex, FIELD_MAP.repetitions) ?? 0);
    const dur = toSeconds(pickFirst(ex, FIELD_MAP.temps) ?? 0);
...
/* ---------------- Programme name (i18n helpers already in file) ---------------- */
...
/* =========================
   ✅ Suivi auto (même logique que ProgramView)
   ========================= */
...
export default function AutoProgramPreview() {
  ...
  // ✅ Suivi auto (bouton identique ProgramView)
  ...
  // ---- Nom "stocké" ----
  const storedProgramName = useMemo(() => {
    const raw =
      prog?.nomProgramme ??
      prog?.nom_programme ??
      prog?.programmeName ??
  ...
  const programmeNameRawForParsing =
    storedProgramName || prog?.nom || prog?.name || prog?.title || t("autoPreview.generated", "Programme");

  const objectifKeyFromName = useMemo(() => {
    return extractObjectifKeyFromNomProgrammeSmart(programmeNameRawForParsing, t) || "";
  }, [programmeNameRawForParsing, t]);
  ...
  const objectifKeyDisplay = useMemo(() => {
    ...
    const ui = (prog?.objectifUI || prog?.options?.objectifUI || prog?.questionnaire?.objectifUI || "").toString().trim();
    if (ui) return ui;
  ...
  const nbSeances = useMemo(() => {
    ...
    const parsed = extractNbSeancesFromNomProgramme(programmeNameRawForParsing);
    if (Number.isFinite(parsed) && parsed > 0) return parsed;

    return null;
  }, [prog, programmeNameRawForParsing, sessions, nbSeancesFromNav]);

  /* =========================
     ✅ TITRE À AFFICHER (SAME as ProgramBuilder)
     - si nomProgramme vide => defaultName
     - si nomProgramme "legacy auto" => defaultName
     - sinon => nomProgramme
     ========================= */
  const programmeTitleDisplay = useMemo(() => {
    const n = Number(nbSeances) || (Array.isArray(sessions) ? sessions.length : 1) || 1;

    const objectifUIKey = String(
      prog?.objectifUI || prog?.options?.objectifUI || prog?.questionnaire?.objectifUI || ""
    ).trim();

    const objectifFallback = String(
      prog?.objectif ||
      prog?.options?.objectif ||
      prog?.questionnaire?.objectif ||
      objectifKeyFromName ||
      ""
    ).trim();

    const defName = makeDefaultProgramName(objectifUIKey, objectifFallback, n);

    if (!storedProgramName) return defName;
    if (isLegacyAutoName(storedProgramName, objectifUIKey, objectifFallback, n)) return defName;

    return storedProgramName;
  }, [prog, storedProgramName, nbSeances, sessions, objectifKeyFromName]);
  ...
  /* ✅ Suivi auto: sync depuis Firestore */
  useEffect(() => {
    if (!prog) return;
  ...
  const renderPdfPages = () => {
    ...
    const Header = ({ sessionIdx, showSessionTitle }) => {
      const leftLabel = viewerIsCoach
        ? (getPrettyUserName(user) || (user?.displayName && !/@/.test(user.displayName) ? user.displayName : "") || "BYL")
  ...
  // ✅ bouton auto-suivi: POUR TOUT LE MONDE (identique ProgramView)
  ...
  return (
    ...
    >
      ...
      <HStack spacing={3} justify={{ base: "flex-start", md: "flex-end" }} wrap="wrap">
          ...
          <Tooltip
            ...
            label={
              autoFollow
                ...
                : t("autoPreview.autoFollowHintOff", "IA désactivée : aucune progression automatique n’est appliquée.")
            }
          >
            <Button
              size="sm"
        ...
        <Select size="sm" w={{ base: "180px", md: "200px" }} value={pdfLang} onChange={(e) => setPdfLang(e.target.value)}>
          {options.map((o) => (
